...
<template>
  ...
  <section class="table-panel"><table><thead><tr><th>Location</th><th>Device</th><th>Status</th><th>Last seen</th><th></th></tr></thead><tbody><tr v-for="m in mirrors" :key="m.id"><td><strong>{{m.location_name}}</strong><small>{{m.public_id}}</small></td><td>{{m.device_name||'Not paired'}}</td><td><StatusPill :value="m.status" /></td><td>{{m.last_seen_at?new Date(m.last_seen_at).toLocaleString():'Never'}}</td><td><button class="text-btn" @click="rotate(m)">Rotate code</button></td></tr></tbody></table></section>
</template>
