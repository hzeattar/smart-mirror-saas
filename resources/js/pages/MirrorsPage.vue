...
<template>
  ...
  <section class="table-panel"><table><thead><tr><th>Location</th><th>Device</th><th>Status</th><th>Last seen</th><th>Latest AI</th><th></th></tr></thead><tbody><tr v-for="m in mirrors" :key="m.id"><td><strong>{{m.location_name}}</strong><small>{{m.public_id}}</small></td><td>{{m.device_name||'Not paired'}}</td><td><StatusPill :value="m.status" /></td><td>{{m.last_seen_at?new Date(m.last_seen_at).toLocaleString():'Never'}}</td><td><template v-if="m.latest_try_on_batch"><StatusPill :value="m.latest_try_on_batch.status" /><small>{{m.latest_try_on_batch.completed_count}}/{{m.latest_try_on_batch.outfit_count}} outfits</small></template><template v-else-if="m.latest_try_on_job"><StatusPill :value="m.latest_try_on_job.status" /><small>{{new Date(m.latest_try_on_job.created_at).toLocaleString()}}</small></template><span v-else class="muted">None</span></td><td><button class="text-btn" @click="rotate(m)">Rotate code</button></td></tr></tbody></table></section>
</template>
